...
coinName = "btc";
url = 'https://crix-api-endpoint.upbit.com/v1/crix/candles/days?code=CRIX.UPBIT.KRW-' + coinName + '&count=1000';
...
#    excel.Quit()                       # 끝나고 엑셀을 닫고 싶을 때
    ...
    for dailyData in data:
        date = dailyData['candleDateTimeKst']
        onlyDate = date.split('T')  # 날짜정보와 시간정보 분리
        print(onlyDate[0], "%d" % dailyData['openingPrice'], "%d" % dailyData['highPrice'],
              "%d" % dailyData['lowPrice'], "%d" % dailyData['tradePrice'], "%d" % dailyData['candleAccTradeVolume'])
